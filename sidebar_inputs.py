...
def sidebar_coil_inputs(coil_idx, coil_state):
    ...
    # Master Coil Info
    coil_state['master_coil_width'] = st.sidebar.number_input(f'Master Coil Width #{coil_idx+1}', min_value=0.0, value=coil_state['master_coil_width'], step=1.0, key=f'master_coil_width_{coil_idx}')
    ...
    coil_state['master_coil_cost'] = st.sidebar.number_input(f'Master Coil Cost #{coil_idx+1}', min_value=0.0, value=coil_state['master_coil_cost'], step=1.0, key=f'master_coil_cost_{coil_idx}')
    ...
    with st.sidebar.expander(f'Examine Coil #{coil_idx+1} (optional)'):
        ...
        if coil_state['examine_cost_check'] and not coil_state['examine_skip']:
            coil_state['examine_cost'] = st.number_input(f'Examine Coil Cost #{coil_idx+1}', min_value=0.0, value=coil_state['examine_cost'], step=1.0, disabled=coil_state['examine_skip'], key=f'examine_cost_{coil_idx}')
    with st.sidebar.expander(f'Trimming #{coil_idx+1} (optional)'):
        ...
        coil_state['trimming_width_cropped_check'] = st.checkbox(f'Set Width Cropped (default: 1) #{coil_idx+1}', disabled=coil_state['trimming_skip'], key=f'trimming_width_cropped_check_{coil_idx}', value=coil_state['trimming_width_cropped_check'])
        if coil_state['trimming_width_cropped_check'] and not coil_state['trimming_skip']:
            coil_state['trimming_width_cropped'] = st.number_input(f'Trimming Width Cropped #{coil_idx+1}', min_value=0.0, value=coil_state['trimming_width_cropped'], step=0.01, disabled=coil_state['trimming_skip'], key=f'trimming_width_cropped_{coil_idx}')
        ...
        if coil_state['trimming_cost_check'] and not coil_state['trimming_skip']:
            coil_state['trimming_cost'] = st.number_input(f'Trimming Cost #{coil_idx+1}', min_value=0.0, value=coil_state['trimming_cost'], step=1.0, disabled=coil_state['trimming_skip'], key=f'trimming_cost_{coil_idx}')
    with st.sidebar.expander(f'Pickle & Oil #{coil_idx+1} (optional)'):
        ...
        if coil_state['pickle_cost_check'] and not coil_state['pickle_skip']:
            coil_state['pickle_cost'] = st.number_input(f'Pickle & Oil Cost #{coil_idx+1}', min_value=0.0, value=coil_state['pickle_cost'], step=1.0, disabled=coil_state['pickle_skip'], key=f'pickle_cost_{coil_idx}')
    with st.sidebar.expander(f'Coating #{coil_idx+1} (optional)'):
        ...
        if coil_state['coating_cost_check'] and not coil_state['coating_skip']:
            coil_state['coating_cost'] = st.number_input(f'Coating Cost #{coil_idx+1}', min_value=0.0, value=coil_state['coating_cost'], step=1.0, disabled=coil_state['coating_skip'], key=f'coating_cost_{coil_idx}')
    with st.sidebar.expander(f'Slitting #{coil_idx+1} (optional)'):
        ...
        for i in range(len(widths_per_cut)):
            with cols[0]:
                if over_width:
                    widths_per_cut[i] = st.number_input(f'Width per Cut #{i+1} (Coil {coil_idx+1})', min_value=0.0, value=widths_per_cut[i], step=0.01, key=f'slitting_width_per_cut_{coil_idx}_{i}', disabled=coil_state['slitting_skip'], format="%.2f", help='Sum of widths exceeds master coil width', label_visibility='visible')
                    ...
                    widths_per_cut[i] = st.number_input(f'Width per Cut #{i+1} (Coil {coil_idx+1})', min_value=0.0, value=widths_per_cut[i], step=0.01, key=f'slitting_width_per_cut_{coil_idx}_{i}', disabled=coil_state['slitting_skip'], format="%.2f")
            with cols[1]:
                num_cuts_needed[i] = st.number_input(f'Num Cuts #{i+1} (Coil {coil_idx+1})', min_value=1, value=num_cuts_needed[i], step=1, key=f'slitting_num_cuts_needed_{coil_idx}_{i}', disabled=coil_state['slitting_skip'])
            with cols[2]:
        ...
        st.markdown(f"**Width Cropped (from Trimming):** {coil_state['trimming_width_cropped'] if coil_state['trimming_width_cropped_check'] and not coil_state['trimming_skip'] else 1.0}")
        coil_state['slitting_width_cropped'] = coil_state['trimming_width_cropped'] if coil_state['trimming_width_cropped_check'] and not coil_state['trimming_skip'] else 1.0
        ...
        if coil_state['slitting_cost_check'] and not coil_state['slitting_skip']:
            coil_state['slitting_cost'] = st.number_input(f'Slitting Cost #{coil_idx+1}', min_value=0.0, value=coil_state['slitting_cost'], step=1.0, disabled=coil_state['slitting_skip'], key=f'slitting_cost_{coil_idx}')
    with st.sidebar.expander(f'Cut to Length #{coil_idx+1} (optional)'):
        ...
        if coil_state['cut_cost_check'] and not coil_state['cut_skip']:
            coil_state['cut_cost'] = st.number_input(f'Cut to Length Cost #{coil_idx+1}', min_value=0.0, value=coil_state['cut_cost'], step=1.0, disabled=coil_state['cut_skip'], key=f'cut_cost_{coil_idx}')
    ...
    coil_state['storage_start'] = st.sidebar.number_input(f'Storage at Start #{coil_idx+1}', min_value=0.0, value=coil_state['storage_start'], step=0.01, key=f'storage_start_{coil_idx}')
    coil_state['storage_examine'] = st.sidebar.number_input(f'Storage after Examine #{coil_idx+1}', min_value=0.0, value=coil_state['storage_examine'], step=0.01, key=f'storage_examine_{coil_idx}')
    coil_state['storage_trimming'] = st.sidebar.number_input(f'Storage after Trimming #{coil_idx+1}', min_value=0.0, value=coil_state['storage_trimming'], step=0.01, key=f'storage_trimming_{coil_idx}')
    coil_state['storage_pickle'] = st.sidebar.number_input(f'Storage after Pickle & Oil #{coil_idx+1}', min_value=0.0, value=coil_state['storage_pickle'], step=0.01, key=f'storage_pickle_{coil_idx}')
    coil_state['storage_coating'] = st.sidebar.number_input(f'Storage after Coating #{coil_idx+1}', min_value=0.0, value=coil_state['storage_coating'], step=0.01, key=f'storage_coating_{coil_idx}')
    coil_state['storage_slitting'] = st.sidebar.number_input(f'Storage after Slitting #{coil_idx+1}', min_value=0.0, value=coil_state['storage_slitting'], step=0.01, key=f'storage_slitting_{coil_idx}')
    coil_state['storage_cut'] = st.sidebar.number_input(f'Storage after Cut to Length #{coil_idx+1}', min_value=0.0, value=coil_state['storage_cut'], step=0.01, key=f'storage_cut_{coil_idx}')
    coil_state['storage_end'] = st.sidebar.number_input(f'Storage at End #{coil_idx+1}', min_value=0.0, value=coil_state['storage_end'], step=0.01, key=f'storage_end_{coil_idx}')
    # Freight
    st.sidebar.header(f'Freight Costs (per step) #{coil_idx+1}')
    coil_state['freight_start'] = st.sidebar.number_input(f'Freight at Start #{coil_idx+1}', min_value=0.0, value=coil_state['freight_start'], step=1.0, key=f'freight_start_{coil_idx}')
    coil_state['freight_examine'] = st.sidebar.number_input(f'Freight after Examine #{coil_idx+1}', min_value=0.0, value=coil_state['freight_examine'], step=1.0, key=f'freight_examine_{coil_idx}')
    coil_state['freight_trimming'] = st.sidebar.number_input(f'Freight after Trimming #{coil_idx+1}', min_value=0.0, value=coil_state['freight_trimming'], step=1.0, key=f'freight_trimming_{coil_idx}')
    coil_state['freight_pickle'] = st.sidebar.number_input(f'Freight after Pickle & Oil #{coil_idx+1}', min_value=0.0, value=coil_state['freight_pickle'], step=1.0, key=f'freight_pickle_{coil_idx}')
    coil_state['freight_coating'] = st.sidebar.number_input(f'Freight after Coating #{coil_idx+1}', min_value=0.0, value=coil_state['freight_coating'], step=1.0, key=f'freight_coating_{coil_idx}')
    coil_state['freight_slitting'] = st.sidebar.number_input(f'Freight after Slitting #{coil_idx+1}', min_value=0.0, value=coil_state['freight_slitting'], step=1.0, key=f'freight_slitting_{coil_idx}')
    coil_state['freight_cut'] = st.sidebar.number_input(f'Freight after Cut to Length #{coil_idx+1}', min_value=0.0, value=coil_state['freight_cut'], step=1.0, key=f'freight_cut_{coil_idx}')
    coil_state['freight_end'] = st.sidebar.number_input(f'Freight at End #{coil_idx+1}', min_value=0.0, value=coil_state['freight_end'], step=1.0, key=f'freight_end_{coil_idx}') 